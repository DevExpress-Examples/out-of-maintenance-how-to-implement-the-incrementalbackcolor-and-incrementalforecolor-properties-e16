...
Namespace Q212749
	...
	Partial Public Class nwindDataSet
		...
		Partial Public Class ProductsDataTable
			...
			<Global.System.Diagnostics.DebuggerNonUserCodeAttribute()> _
			Public Function AddProductsRow(ByVal ProductName As String, ByVal QuantityPerUnit As String, ByVal UnitPrice As Decimal) As ProductsRow
				Dim rowProductsRow As ProductsRow = (CType(Me.NewRow(), ProductsRow))
				Dim columnValuesArray() As Object = { Nothing, ProductName, QuantityPerUnit, UnitPrice}
	...
	End Class
End Namespace
Namespace Q212749.nwindDataSetTableAdapters
	...
	Partial Public Class ProductsTableAdapter
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `Products` WHERE ((`ProductID` = ?) AND ((? = 1 AND `ProductName` IS " & "NULL) OR (`ProductName` = ?)) AND ((? = 1 AND `QuantityPerUnit` IS NULL) OR (`Qu" & "antityPerUnit` = ?)) AND ((? = 1 AND `UnitPrice` IS NULL) OR (`UnitPrice` = ?)))" & ""
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `Products` SET `ProductName` = ?, `QuantityPerUnit` = ?, `UnitPrice` = ? WHERE ((`ProductID` = ?) AND ((? = 1 AND `ProductName` IS NULL) OR (`ProductName` = ?)) AND ((? = 1 AND `QuantityPerUnit` IS NULL) OR (`QuantityPerUnit` = ?)) AND ((? = 1 AND `UnitPrice` IS NULL) OR (`UnitPrice` = ?)))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT ProductID, ProductName, QuantityPerUnit, UnitPrice FROM Products"
		...
		Public Overridable Function Delete(ByVal Original_ProductID As Integer, ByVal Original_ProductName As String, ByVal Original_QuantityPerUnit As String, ByVal Original_UnitPrice As Global.System.Nullable(Of Decimal)) As Integer
			...
			Else
				...
				Me.Adapter.DeleteCommand.Parameters(4).Value = (CStr(Original_QuantityPerUnit))
			End If
		...
		Public Overridable Function Update(ByVal ProductName As String, ByVal QuantityPerUnit As String, ByVal UnitPrice As Global.System.Nullable(Of Decimal), ByVal Original_ProductID As Integer, ByVal Original_ProductName As String, ByVal Original_QuantityPerUnit As String, ByVal Original_UnitPrice As Global.System.Nullable(Of Decimal)) As Integer
			...
			Else
				Me.Adapter.UpdateCommand.Parameters(1).Value = (CStr(QuantityPerUnit))
			End If
			...
			Else
				...
				Me.Adapter.UpdateCommand.Parameters(7).Value = (CStr(Original_QuantityPerUnit))
			End If
			If (Original_UnitPrice.HasValue = True) Then
